...
"""
论文数据库Web管理界面
使用Flask提供简单的Web界面来管理论文数据库
"""
from flask import Flask, render_template_string, request, jsonify, redirect, url_for
import sqlite3
import json
from paper_db_manager import PaperDatabaseManager
...
<body>
    <div class="container">
        ...
        <div id="dashboard" class="tab-content active">
            <div class="stats">
                ...
                <div class="stat-card" style="background: #ffc107; color: #333;">
                    <h3>{{ "%.1f"|format(stats.file_sizes.total_mb) }} MB</h3>
                    <p>总文件大小</p>
                </div>
                {% endif %}
            </div>
        ...
        <div id="import" class="tab-content">
            <div style="background: #f8f9fa; padding: 20px; border-radius: 6px;">
                <h3>📥 导入CSV数据</h3>
                <p>将CSV文件放在服务器目录中，然后点击下方按钮导入:</p>
                
                <form method="POST" action="/import" style="margin-top: 15px;">
                    <input type="text" name="csv_path" placeholder="CSV文件路径 (如: index.csv)" required style="width: 300px; padding: 10px; margin-right: 10px;">
                    <button type="submit" style="padding: 10px 20px; background: #28a745; color: white; border: none; border-radius: 4px;">导入数据</button>
                </form>
                
                <div style="margin-top: 20px; padding: 15px; background: #e7f3ff; border-radius: 4px;">
                    <strong>💡 提示:</strong>
                    <ul>
                        <li>CSV文件应包含以下列: Title, Year, Venue, Category, PDF, File</li>
                        <li>系统会自动检查重复记录并更新</li>
                        <li>导入过程中会验证文件下载状态</li>
                    </ul>
                </div>
    ...
    <script>
        ...
        window.addEventListener('load', function() {
            ...
            if (tab && ['dashboard', 'search', 'import'].includes(tab)) {
                const tabs = document.querySelectorAll('.nav-tab');
                tabs.forEach(t => t.classList.remove('active'));
                document.querySelectorAll('.tab-content').forEach(c => c.classList.remove('active'));
                
                const tabIndex = ['dashboard', 'search', 'import'].indexOf(tab);
...
def import_csv():
    ...
    if csv_path:
        ...
        if success:
            return redirect(url_for('index', tab='import', msg='success'))
        else:
            return redirect(url_for('index', tab='import', msg='error'))
...
def api_search():
    ...
    papers = db_manager.search_papers(query, category, year, limit)
    ...
    for paper in papers:
        paper_dict = {
            ...
            'file_path': paper[6],
            'file_size': paper[7],
            'download_status': paper[8],
            'download_date': paper[9],
